...
export function scanFileSkills() {
    const map = new Map();
...
export async function getSkillPath(ctx, name) {
...
export async function setSkillInvocation(ctx, name, opts) {
...
export async function removeSkill(ctx, name, trashDir) {
    const path = await getSkillPath(ctx, name);
    if (!path) {
 ...
 */
export function editFrontmatter(text, changes) {
    const lines = text.split(/\r?\n/);
    let inFront = false;
    let frontEnd = -1;
    if (lines[0]?.trim() === '---') {
