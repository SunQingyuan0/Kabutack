window.__ModuleLoader__.load({ id: "@dsh-external/kabutack", factory: (require) => {
...
function createKabutackPanel() {
    ...
    const renderCatalog = () => {
        ...
        let currentKind = 'plugin';
        const kindButtons = [];
        const kinds = [['plugin', '插件'], ['skill', 'Skill'], ['mcp', 'MCP']];
        ...
        const draw = () => {
            for (const node of Array.from(catalogPanel.querySelectorAll('.kbt-section')))
                node.remove();
            ...
            if (currentKind === 'plugin') {
                const items = catalog.plugins
                    .map((p) => ({ ...p, title: p.moduleName, stateText: p.enabled ? '运行中' : '已停用', stateCls: p.enabled ? 'on' : 'off' }))
                    .filter((x) => matches(x.title));
                section.append(renderItems(items, 'plugin'));
            }
            else if (currentKind === 'skill') {
                const items = catalog.skills
                    .map((s) => ({ ...s, title: s.name, stateText: (s.modelInvocable ? 'M' : '-') + '/' + (s.userInvocable ? 'U' : '-'), stateCls: s.modelInvocable || s.userInvocable ? 'on' : 'off' }))
                    .filter((x) => matches(x.title));
                ...
                const items = catalog.mcps
                    .map((m) => ({ ...m, title: m.serverName, stateText: m.enabled ? '运行中' : '已停用', stateCls: m.enabled ? 'on' : 'off' }))
                    .filter((x) => matches(x.title));
    ...
    const openMcpModal = () => {
        ...
        const serverInput = el('input', 'kbt-input');
        serverInput.placeholder = 'serverName';
        const transportSelect = el('select', 'kbt-input');
        for (const [value, label] of [['stdio', 'stdio'], ['streamable-http', 'streamable-http']]) {
            const opt = el('option', undefined, label);
            opt.value = value;
            transportSelect.append(opt);
        }
        const commandInput = el('input', 'kbt-input');
        commandInput.placeholder = 'command (stdio)';
        const urlInput = el('input', 'kbt-input');
        urlInput.placeholder = 'url (streamable-http)';
        const argsInput = el('input', 'kbt-input');
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (!serverName)
                return say('MCP serverName 必填', true);
            const transport = transportSelect.value;
            const payload = { serverName, transport };
            if (transport === 'stdio') {
                if (!commandInput.value.trim())
                    return say('stdio 需要 command', true);
                payload.command = commandInput.value.trim();
                payload.args = argsInput.value.split(',').map((s) => s.trim()).filter(Boolean);
            }
            else {
                if (!urlInput.value.trim())
                    return say('streamable-http 需要 url', true);
    ...
    const openRoleModal = (roleId) => {
        ...
        const originalPlugins = catalog.plugins
            .filter((p) => !p.moduleName.startsWith('@dsh-external/'))
            .map((p) => p.moduleName);
        const selectedPlugins = new Set(role ? role.plugins || [] : originalPlugins);
        const selectedSkills = new Set(role ? role.skills || [] : catalog.skills.map((s) => s.name));
        const selectedMcps = new Set(role ? role.mcps || [] : []);
        const makeGrid = (items, selected) => {
        ...
        body.append(makeGrid(catalog.plugins.map((p) => ({ id: p.moduleName, label: p.moduleName })), selectedPlugins));
        body.append(el('h4', 'kbt-section-title', 'Skill'));
        body.append(makeGrid(catalog.skills.map((s) => ({ id: s.name, label: s.name })), selectedSkills));
        body.append(el('h4', 'kbt-section-title', 'MCP'));
        body.append(makeGrid(catalog.mcps.map((m) => ({ id: m.serverName, label: m.serverName })), selectedMcps));
        const close = () => overlay.remove();
...
function KabutackSection() {
    const ref = React.useRef(null);
    React.useEffect(() => {
        const panel = createKabutackPanel();
        if (ref.current)
            ref.current.appendChild(panel.element);
...
function apply(ctx) {
    ctx.effect(() => ctx.slots.inject('settings.section', () => ctx.slots.register({
        ...
        order: 60,
        label: () => 'Kabutack',
    }, KabutackSection)), '@dsh-external/kabutack: settings page');
}
