...
export function buildPlan(role, catalog, store) {
    const warnings = [];
    const plan = {
        roleId: role.id,
        enablePlugins: [],
...
export async function executePlan(ctx, plan, catalog, store, save) {
    const executed = [];
    const rolledBack = [];
    const undoStack = [];
    const run = async (label, action, undo) => {
        try {
            await action();
            executed.push(label);
            undoStack.push(undo);
    ...
    try {
        ...
        for (const item of plan.enablePlugins) {
            if (item.moduleName === '@deepseek-ai/dsh-mcp-client')
                continue;
            await run('enable-plugin:' + item.moduleName, () => setPluginEnabled(ctx, item.entryId, true), () => setPluginEnabled(ctx, item.entryId, false));
        }
        // 2. 停用插件
        for (const item of plan.disablePlugins) {
            await run('disable-plugin:' + item.moduleName, () => setPluginEnabled(ctx, item.entryId, false), () => setPluginEnabled(ctx, item.entryId, true));
        }
        // 2.5 启用已有 MCP
        for (const item of plan.enableMcps) {
            await run('enable-mcp:' + item.serverName, () => setPluginEnabled(ctx, item.entryId, true), () => setPluginEnabled(ctx, item.entryId, false));
        }
        // 3. 创建 MCP
        for (const def of plan.createMcps) {
            await run('create-mcp:' + def.serverName, async () => {
                ...
                const target = entries.find((e) => !e.options.group && e.options.name === '@deepseek-ai/dsh-mcp-client' && e.options.config?.serverName === def.serverName);
                if (target)
                    await removeEntry(ctx, target.id);
        ...
        for (const item of plan.removeMcps) {
            ...
            await run('remove-mcp:' + item.serverName, async () => {
        ...
        for (const item of plan.enableSkills) {
            ...
            await run('enable-skill:' + item.name, async () => { await setSkillInvocation(ctx, item.name, { modelInvocable: true, userInvocable: true }); }, async () => {
                await setSkillInvocation(ctx, item.name, {
        ...
        for (const item of plan.disableSkills) {
            const prev = catalog.skills.find((s) => s.name === item.name);
            await run('disable-skill:' + item.name, async () => { await setSkillInvocation(ctx, item.name, { modelInvocable: false, userInvocable: false }); }, async () => {
                await setSkillInvocation(ctx, item.name, {
                    modelInvocable: prev?.modelInvocable ?? true,
                    userInvocable: prev?.userInvocable ?? true,
                });
