...
export function createRoleManagerService(services: KabutackServices): RoleManagerService {
  const { ctx } = services
  const getStore = services.getStore
  const save = services.save
  const audit = services.audit

  const removePluginByModuleName = async (moduleName: string): Promise<void> => {
    ...
    if (!entry) throw new Error('not-found: 插件不存在: ' + moduleName)
  ...
  const removeManagedCapability = async (kind: 'plugin' | 'mcp', id: string): Promise<void> => {
    if (kind === 'plugin') {
  ...
  return {
    ...
    async setPluginEnabled(entryId, enabled) {
      ...
      if (!entry) throw new Error('not-found: 插件 entry 不存在: ' + entryId)
