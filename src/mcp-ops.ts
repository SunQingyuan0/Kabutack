...
export function validateMcpDefinition(def: McpDefinition): void {
  if (!def?.serverName || !SERVER_NAME_RE.test(def.serverName)) {
    throw new Error('invalid-input: serverName 必须为 [A-Za-z0-9_-]{1,32}')
  }
  if (def.transport === 'stdio') {
    if (!def.command) throw new Error('invalid-input: stdio 传输需要 command')
  } else if (def.transport === 'streamable-http') {
    if (!def.url) throw new Error('invalid-input: streamable-http 传输需要 url')
  } else {
    throw new Error('invalid-input: transport 必须是 stdio 或 streamable-http')
  }
}

export function toMcpItem(ctx: any, entry: any): McpItem | undefined {
  const config: McpDefinition | undefined = entry?.options?.config
  if (!config?.serverName) return undefined
  return {
    kind: 'mcp',
    entryId: entry.id,
...
export function listMcps(ctx: any): McpItem[] {
  return findMcpEntries(ctx).map((e) => toMcpItem(ctx, e)).filter((x): x is McpItem => Boolean(x))
}

export function findMcpEntries(ctx: any): any[] {
  if (!ctx?.loader?.entries) return []
  return [...ctx.loader.entries()].filter((e: any) => !e.options.group && e.options.name === '@deepseek-ai/dsh-mcp-client')
}

export async function addMcp(ctx: any, store: RoleManagerStore, def: McpDefinition, save: (s: RoleManagerStore) => void): Promise<McpItem> {
...
export async function updateMcp(ctx: any, store: RoleManagerStore, serverName: string, patch: Partial<McpDefinition>, save: (s: RoleManagerStore) => void): Promise<McpItem> {
  const existing = store.mcps.find((m) => m.serverName === serverName)
  if (!existing) throw new Error('not-found: MCP 定义不存在: ' + serverName)
  const merged: McpDefinition = { ...existing, ...patch, serverName }
...
export async function removeMcp(ctx: any, store: RoleManagerStore, serverName: string, save: (s: RoleManagerStore) => void): Promise<void> {
  const entry = findEntryByServerName(ctx, serverName)
  if (entry) await removeEntry(ctx, entry.id)
  const idx = store.mcps.findIndex((m) => m.serverName === serverName)
  if (idx >= 0) store.mcps.splice(idx, 1)
  save(store)
}

export async function setMcpEnabled(ctx: any, serverName: string, enabled: boolean): Promise<void> {
  const entry = findEntryByServerName(ctx, serverName)
  if (!entry) throw new Error('not-found: MCP 不存在: ' + serverName)
